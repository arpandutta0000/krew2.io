...
var refreshTimer = 0; // used to handle refreshing krew list
var playerModels = [];
...
var setPlayerModels = function () {
    ...
    var krewmateModel = new THREE.Mesh(geometry.krewmate, materials.krewmate);
...
Player.prototype.clientlogic = function (dt) {
    ...
    if (
        ...
        var island = entities[this.parent.anchorIslandId || this.parent.id];
        var islandPosition = new THREE.Vector3(0, 0, 0);
        ...
        if (this.isCaptain) {
            ...
            if (this.label !== undefined) {
                this.label.material.color =
                    Admins.includes(this.name) || Mods.includes(this.name) || Devs.includes(this.name) ? labelcolors.staff :
                    this.isPlayer ?
                    labelcolors.myself :
                    this.isCaptain ?
                    labelcolors.captain :
            ...
            if (this.label !== undefined) {
                this.label.material.color = Admins.includes(this.name) || Mods.includes(this.name) || Devs.includes(this.name) ? labelcolors.staff :
                    this.isPlayer ?
                    labelcolors.myself :
                    this.isCaptain ?
                    labelcolors.captain :
