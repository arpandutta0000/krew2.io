...
setInterval(() => {
    ...
    for(let i in core.players) {
        ...
        scores.players.push({
            ...
            cU: player.cargoused,
            sS: player.shipsSank,
            ok: player.parent ? player.parent.overallKills: undefined,
            oc: player.parent ? player.parent.overallCargo: undefined,
            oql: player.parent ? player.parent.otherQuestLevel: undefined,
    ...
    if(chestCount == 0) {
        if(chestRespawn < Date.now()) core.createPickup(4, core.worldsize * Math.random(), core.worldsize * Math.random(), 4, false, (Math.random() * 5e4) + 1e4);
        else if(chestRespawn == undefined) chestRespawn = Date.now() + (Math.random() * 6e5) + 3e5;
    }
